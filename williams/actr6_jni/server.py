...
class JNI_Server(Factory):
	...
	def sendCommand(self, *args, **kwargs):
		if self.p:
			self.p.sendCommand(*args, **kwargs)
	
	def add_dm(self, chunk):
		self.sendCommand(self.model, "add-dm", chunk=chunk.get_chunk())
	...
	def update_display(self, chunks, clear=False):
		...
		args = {"loc-chunks": visual_locations, 
				...
				"clear": clear}
		self.sendCommand(self.model, "update-display", **args)
	...
	def display_new(self, chunks):
		...
		args = {"loc-chunks": visual_locations, 
				"obj-chunks": visual_objects}
		self.sendCommand(self.model, "display-new", **args)
	...
	def display_add(self, chunk):
		...
		self.sendCommand(self.model, "display-add", **args)
	...
	def display_remove(self, chunk=None, name=None):
		...
		if name:
			...
			self.sendCommand(self.model, "display-remove", **args)
	...
	def display_update(self, chunks, clear=False):
		...
		self.sendCommand(self.model, "display-update", chunks=chunks, clear=clear)
		
	def set_cursor_location(self, loc):
		self.sendCommand(self.model, "set-cursor-loc", loc=loc)

	def digit_sound(self, digit):
		self.sendCommand(self.model, "new-digit-sound", digit=digit)

	def tone_sound(self, freq, duration):
		self.sendCommand(self.model, "new-tone-sound", frequency=freq, duration=duration)

	def word_sound(self, words):
		self.sendCommand(self.model, "new-word-sound", words=words)

	def other_sound(self, content, onset, delay, recode):
		self.sendCommand(self.model, "new-other-sound", content=content, onset=onset, delay=delay, recode=recode)

	def trigger_reward(self, reward):
		self.sendCommand(self.model, "trigger-reward", reward=reward)

	def disconnect(self):
		self.sendCommand(self.model, "disconnect")
		
	def setup(self, width, height):
		self.sendCommand(self.model, "setup", width=width, height=height)
		
	def trigger_event(self, event, *args):
		self.sendCommand(self.model, "trigger-event", event=event, args=args)
